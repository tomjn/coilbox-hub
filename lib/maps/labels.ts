...
/**
 * A map's measurements in the words players use for them (#190).
 *
 * `public.map` stores a size in elmos and a set of start positions. Nobody says
 * either out loud. They say "a 12 by 20" and "an 8 player map", and those two
 * sentences are what a page has to print.
 *
 * Here rather than in the page, because the map listing (#191) prints the same
 * two sentences on every card. Two copies would drift the first time one of them
