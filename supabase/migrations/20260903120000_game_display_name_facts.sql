-- A game's own name, description and links, off its own modinfo (#300).
--
-- public.game.display_name and .description have sat null on every row since
-- 20260821100000_game_catalog.sql, and .links has sat at '[]', because the
-- only writer of any of them was submit_game_facts inserting the literal
-- nulls and empty array the first time it saw a shortname, and nothing in the
-- function has ever set any of them afterwards. The only other writer is
-- editGameDetails (app/games/actions.ts), an owner using the edit form, and
-- no game has a claimed owner yet.
--
-- coilbox now sends what the game calls itself (tomjn/coilbox#1950), so the
-- three columns are filled here, but only when the column is still at its
-- starting value: display_name and description when null, links when it is
-- still an empty array. Anything already held, whether from an earlier
-- report or from an owner's own edit, is left alone. That is what lets a
-- sweep run forever without ever arguing with somebody who has since claimed
-- the game and typed a better name in by hand.
--
-- A new migration carrying the whole function rather than an edit of the
-- applied one, per the house rule. Stacked on 20260830120000_game_unit_morph.sql,
-- the true current body; this is that function plus the three columns,
-- unchanged elsewhere.
...
begin
  ...
  -- The game's own name and description, and the links coilbox curated for it
  -- (#300). Each fills only the gap it finds: display_name and description
  -- when the column is still null, links when it is still the empty array
  -- every game starts with. A value already held, from an earlier report or
  -- from an owner's own edit, is never replaced.
  v_links := case
    when jsonb_typeof(p_submission -> 'links') = 'array' then p_submission -> 'links'
    else null
  end;
  update public.game as g
  set
    display_name = coalesce(g.display_name, p_submission ->> 'name'),
    description = coalesce(g.description, p_submission ->> 'description'),
    links = case
      when jsonb_array_length(g.links) = 0 and v_links is not null and jsonb_array_length(v_links) > 0
        then v_links
      else g.links
    end
