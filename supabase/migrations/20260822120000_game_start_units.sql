-- Reading the start units the route actually sends (#246).
--
-- 20260821110000_game_facts.sql read the roots out of the submission as
-- p_submission -> 'startUnits'. Every other key that function reads is the
-- normalised snake case the route hands over - facts_digest, full_name,
-- faction_key, build_options - but this one kept the wire's camel case, so
-- jsonb_typeof(p_submission -> 'startUnits') was always null and neither
-- branch ran. Nothing was stored wrong; it simply never arrived, and a build
-- tree rooted itself at nothing until a client re-sent its backfill.
--
-- The suite shared the blind spot, which is why it stayed green: it submitted
-- camelCase too, agreeing with the function rather than with the route.
-- game_submission.test.sql now submits what submitGameFacts really sends and
-- carries a second report whose roots must replace the first's.
--
-- A new migration rather than an edit of the applied one, per the house rule;
-- the body is otherwise the function as it stood.
