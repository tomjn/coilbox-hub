-- Answering every faction a submission carries (#247).
--
-- submit_game_facts promised in its own doc comment to return one result row
-- per faction and unit, and lib/games/submit.ts zips exactly that shape back
-- into the response - but the SQL only ever built rows with kind 'unit'. A
-- faction the tables refused was therefore invisible: the client's debugging
-- surface read as though every side had been accepted.
--
-- The insert also sat outside any subtransaction, so the first faction that
-- could not land - a key named twice in one batch, against
-- game_faction_identity_idx - aborted the whole submission rather than costing
-- itself. Each side now lands in its own block the way a unit does, and every
-- entry is answered before the units are walked.
--
-- Stacked on 20260822120000_game_start_units.sql; this body is that function
-- plus the faction loop, unchanged elsewhere.
